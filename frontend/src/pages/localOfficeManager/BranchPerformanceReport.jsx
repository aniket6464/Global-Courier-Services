...
const BranchPerformanceReport = () => {
  ...
  useEffect(() => {
    const fetchPerformanceData = async () => {
      try {
        const response = await fetch('/api/branch/local-office/performanceByid');
        const branch = await response.json();console.log(branch);
        ...
        const performanceMetrics = {
          ...
          parcel_condition: {
            undamaged_delivery_rate: ((branch.performance_tracking.total_parcels - branch.performance_tracking.damaged_in_transit / branch.performance_tracking.total_parcels) || 0) * 100,
  ...
  return (
    <div className="container mx-auto p-4">
      {/* Page Title */}
      <h1 className="text-2xl font-bold text-center mb-6">Local Office Performance Report</h1>
