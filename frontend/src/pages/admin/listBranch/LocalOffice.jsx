...
const LocalOffice = () => {
  ...
  const [regionalHubFilter, setRegionalHubFilter] = useState("");
  ...
  useEffect(() => {
    const fetchBranchesAndManagers = async () => {
      ...
      let apiUrl = `/api/branch/local-office/read?search=${searchQuery}&sort=${sortBy}&limit=${limit}&page=${currentPage}&main_branch_id=${mainBranchFilter}&regional_hub_id=${regionalHubFilter}`;
      ...
      try {
        ...
        const updatedBranches = await Promise.all(
          data.branches.map(async (branch) => {
            ...
            if (branch.local_office_manager) {
              const managerEndpoint = `/api/user/local-office-manager/read/${branch.local_office_manager}`;
  ...
  }, [searchQuery, sortBy, limit, mainBranchFilter, regionalHubFilter, currentPage]);
  ...
  const fetchManagers = async () => {
    const apiUrl = "/api/user/local-office-manager/read?limit=100";
  ...
  const deleteBranch = async (id) => {
    ...
    try {
      await fetch(`/api/branch/local-office/delete/${id}`, { method: "DELETE" });
  ...
  const assignManager = async () => {
    ...
    const apiUrl = `/api/branch/local-office/change-manager/${selectedBranchId}`;
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex space-x-4 mb-4">
        ...
        <select
        className="border border-gray-300 rounded-md p-2"
        value={regionalHubFilter}
        onChange={(e) => setRegionalHubFilter(e.target.value)}
        >
        <option value="">Filter by Regional Hub</option>
        {regionalHubs.map((hub) => (
            <option key={hub._id} value={hub._id}>
            {hub.branch_code} - {hub.city}
            </option>
        ))}
        </select>
      </div>
            ...
            {branches.length > 0 && branches.map((branch) => (
              <tr key={branch._id}>
                ...
                <td className="py-2 px-4 border-b">
                  {
                    ...
                    <button
                      onClick={() => openModal(branch._id)} // Replace "main" with the actual branch type (main, regional, or local)
                ...
                <td className="py-2 px-4 border-b">
                    ...
                    <button
                      onClick={() => navigate(`/admin/branch-management/update-local/${branch._id}`)}
